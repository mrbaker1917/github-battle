...
export default class Popular extends React.Component {
  ...
  updateLanguage(selectLanguage) {
    ...
    if (!this.state.repos[selectLanguage]) {
      fetchPopularRepos(selectLanguage)
        .then((data) => {
          this.setState(({ repos }) => ({
            repos: {
              repos,
  ...
  isLoading() {
    const { selectLanguage, repos, error } = this.state;
    return !repos[selectLanguage] && error === null;
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        {repos[selectLanguage] && <ReposGrid repos={repos[selectLanguage]} />}
      </React.Fragment>
    );
  }
}
